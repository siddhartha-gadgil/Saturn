...
def FinSeq.delete{α : Type}{n: Nat}(k : Nat) (_ : k < (n + 1)) (seq : FinSeq (n + 1) α): FinSeq n α
...
def findElem?{α: Type}[DecidableEq α]{n: Nat}:
  (seq: FinSeq n  α) → (elem: α) →  Option (ElemInSeq seq elem) :=
   match n with
  | zero => fun _ _ =>  none
...
def elemNotBeyond{α: Type}{n : Nat}(seq : FinSeq n α)(elem : α)(m: Nat): Prop :=
      ∀ k: Nat, ∀ kw : k <n,  m ≤ k →  Not (seq k kw = elem)
...
def searchElem{α: Type}[DecidableEq α]{n: Nat}:
...
def equalBeyond{α: Type}{n : Nat}(seq1 seq2 : FinSeq n α)(m: Nat): Prop :=
  ∀ k: Nat, ∀ kw : k <n, m ≤ k →  seq1 k kw = seq2 k kw
